...
def generate_baseline(output_path: Path):
    ...
    try:
        ...
        # 3. Install dependencies and run tests using the *new* venv's python/pdm
        # We use the full path to the executable to be sure.
        ...
        run_command(f"pdm run pytest --cov=inference_perf --cov-config={config_path.absolute()} --cov-report=json:{output_path.absolute()} tests/", cwd=temp_dir)
