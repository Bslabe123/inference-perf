...
class VisionArenaConfig(BaseModel):
    ...
    max_image_dim: Optional[int] = Field(
        default=None,
        gt=0,
        description=(
            "If set, downscale each image so its longest side is at most this many pixels "
            "(aspect preserved, re-encoded). Omit to send images at their original resolution."
        ),
    )
    insertion_point: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Placement of the image block(s) within the prompt text (0.0=start, 1.0=end).",
    )
