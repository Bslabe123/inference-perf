...
class TestLoadGeneratorPreprocess(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        ...
        self.mock_datagen.get_data.return_value = iter([MagicMock(prefered_worker_id=-1) for _ in range(1000)]) # Mock data generator
    ...
    async def test_preprocess_success(self, mock_sleep, mock_perf_counter) -> None:
        ...
        self.assertTrue(all(s.rate > 0 for s in self.load_generator.stages))
        
        # Verify run_stage was called 3 times
        self.assertEqual(self.load_generator.run_stage.call_count, 3)
