...
class TestPreprocessSaturation(unittest.IsolatedAsyncioTestCase):
    async def test_preprocess_generates_2x_saturation(self):
        # Setup
        ...
        generator.run_stage = AsyncMock(side_effect=mock_run_stage)

        with patch("inference_perf.loadgen.load_generator.time.perf_counter", side_effect=mock_time), \
             patch("inference_perf.loadgen.load_generator.sleep", side_effect=mock_sleep):
            
            await generator.preprocess(
                client, queue, active_counter, finished_counter, request_phase, cancel_signal
            )
            
        # Verify stages
        # Saturation point should be ~100
        # Generated rates should go up to ~200 (2x saturation)
        # Stages are generated using generator.generateRates(saturation * 2.0, num_stages, type)
        # If saturation is 100, max rate should be 200.
        
        self.assertTrue(len(generator.stages) > 0, "No stages generated")
        max_rate = max(stage.rate for stage in generator.stages)
        
        print(f"Generated rates: {[s.rate for s in generator.stages]}")
        
        # We expect max rate to be close to 200
        self.assertAlmostEqual(max_rate, 200.0, delta=5.0)
